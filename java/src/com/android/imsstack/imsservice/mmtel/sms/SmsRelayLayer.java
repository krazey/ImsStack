...
public class SmsRelayLayer {
    ...
    /**
     * Returns the incoming TPDU associated with the given token.
     * @param token the message token.
     * @return SmsTPdu object or null if not found.
     */
    public SmsTPdu getIncomingTpdu(int token) {
        if (mTokenStateTrackerMap.containsKey(token)) {
            SmsRLStateMachine stateMachine = mTokenStateTrackerMap.get(token);
            if (stateMachine != null && stateMachine.getRpduData() != null) {
                return stateMachine.getRpduData().getTpdu();
            }
        }
        return null;
    }
            ...
            try {
                ...
                if (mtData.getMessageType() == SmsUtils.RP_ACK) {
                    synchronized (mLock) {
                        ...
                        mSmsRLStateMachine = mTokenStateTrackerMap.get(token);
                        mSmsRLStateMachine.setRpduData(mtData);
