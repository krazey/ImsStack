...
TEST_F(SipPrivacyHeaderTest, DecodeHdr)
{
    ...
    EXPECT_EQ(SIP_FALSE, pHeader->DecodeHdr((char*)"user;", 5));
    pHeader->SipDelete();
    pHeader = SIP_NULL;
    ...
    /* Decode valid value */
    EXPECT_EQ(SIP_TRUE, pHeader->DecodeHdr((char*)"history", 7));
    ...
    EXPECT_EQ(SIP_TRUE, pHeader->DecodeHdr((char*)"user;header;id", 14));
