...
public class WlanToLteHandoverTest extends CallTestBase {
    ...
    /**
     * Given that the device is registered on WLAN and re-INVITE on handover is enabled.
     * When a voice call is established and a handover from WLAN to LTE occurs.
     * Then the device should send a re-INVITE with P-Access-Network-Info header including
     * "3GPP-E-UTRAN-FDD" to the network to update the access network information.
     */
    @Test
    public void reinviteEnabledOnHo_handover_sendReinvite() throws Exception {
        ...
        // 3. Verify that the P-Access-Network-Info header in INVITE contains "IEEE-802.11".
        // 4. Make the MO call connected.
        // 6. Verify that the P-Access-Network-Info header in re-INVITE contains "3GPP-E-UTRAN-FDD".
        // 7. Disconnect the call from the server after 1 second.
        int timeToHandoverMs = 2000;
        setupScenarioWithReinvite(generator, timeToHandoverMs, PANI_WLAN, PANI_LTE);
        mServerControlConnection.sendControlCommand(generator.build().toString());

        // 1. Complete the normal registration process over WLAN.
        performRegistration(REGISTRATION_TECH_IWLAN);
    ...
    /**
     * Given that the device is registered on WLAN and re-INVITE on handover is disabled.
     * When a voice call is established and a handover from WLAN to LTE occurs.
     * Then the device should not send a re-INVITE to the network, and the subsequent SIP message
     * should have P-Access-Network-Info header including "3GPP-E-UTRAN-FDD".
     */
    @Test
    public void reinviteDisabledOnHo_handover_notSendReinvite() throws Exception {
        ...
        //    "3GPP-E-UTRAN-FDD"
        int timeToHandoverMs = 2000;
        setupScenarioWithoutReinvite(generator, PANI_WLAN, PANI_LTE);
        mServerControlConnection.sendControlCommand(generator.build().toString());
