...
public class BasicLteRegistrationTest extends RegistrationTestBase {
    ...
    @Test
    @P2
    public void register_onLte_withAccessTypeDisabled_verifiesTagNotPresent() throws Exception {
        ...
        //    tag.
        // 4. Trigger IMS registration on the device.
        // 5. Verify that the device successfully registers on LTE.
    ...
    @Test
    @P1
    public void register_onLte_withAccessTypeEnabled_verifiesNumericalTag() throws Exception {
        // 1. Set up the server to expect a REGISTER request and verify its Contact header
    ...
    @Test
    @P2
    public void register_onLte_withAccessTypeWithoutNumericalValue_verifiesTag()
            throws Exception {
        // 1. Set up the server to expect a REGISTER request and verify its Contact header
    ...
    public void register_onLte_with500AsRepeatedOnlyAttachedNetwork_triggersDeregistered()
        ...
        // 1. Set up the server to respond to the REGISTER request with a 500 Server Internal Error.
        // 2. Configure the device via CarrierConfig to define the 5xx response as a 'repeated
        //    error on attached network' type. This error type should not trigger a PLMN block
        //    on a combined-attach network.
        // 3. Trigger IMS registration on the device in an LTE combined-attach network.
        // 4. Verify that the device enters a deregistered state without any suggested action
        //    (e.g., PLMN block).
    ...
    public void register_onLte_with500AsRepeatedOnlyAttachedNetwork_triggersPlmnBlockWithTimeout()
        ...
        // 3. Trigger IMS registration on the device in an LTE EPS-only network.
        // 4. Verify that the device enters a deregistered state with the suggested action to
        //    block the PLMN with a timeout on the LTE network.
