...
public class BasicNrRegistrationTest extends RegistrationTestBase {
    ...
    public void register_onNr_byDisablingVideoCapability_succeeds() throws Exception {
        ...
        //    the video feature tag.
        // 3. After registration, the video capability is disabled on the device.
        // 4. This change triggers a re-registration. The server now expects a second REGISTER
        //    request and verifies its Contact header no longer contains the video feature tag.
        ...
        generator.addMessages("<200-REGISTER | >SUBSCRIBE | <200-SUBSCRIBE");
        generator.addMessage(MessageBuildUtils.getDefaultRegister()
                .addRuleSet(new RuleSet.Builder("REGISTER(2nd) : Valid Contact header")
                        .addRule(new RuleSet.Rule.RuleBuilder("Contact")
                                .addContainRule("+g.3gpp.icsi-ref=\"urn%3Aurn-7%3A3gpp-service"
                                        + ".ims.icsi.mmtel")
                                .addContainRule("audio")
    ...
    public void register_onNr_byEnablingVideoCapability_succeeds() throws Exception {
        ...
        //    include the video feature tag.
        // 3. After successful registration, the video capability is enabled on the device.
        // 4. This change triggers a re-registration. The server now expects a second REGISTER
        //    request and verifies its Contact header includes the video feature tag.
        // 5. Verify that the device successfully re-registers on NR.
    ...
    @Test
    @P2
    public void register_onNr_withAccessTypeDisabled_verifiesTagNotPresent() throws Exception {
        ...
        // 2. The server then completes the registration and subscription flow.
        // 3. Configure the device via CarrierConfig to disable the preferred access type feature
        //    tag.
        // 4. Trigger IMS registration on the device in an NR network.
        // 5. Verify that the device successfully registers on NR.
    ...
    @Test
    @P1
    public void register_onNr_withAccessTypeEnabled_verifiesNumericalTag() throws Exception {
        // 1. Set up the server to expect a REGISTER request and verify its Contact header
        //    includes the '+g.3gpp.accesstype="cellular2"' feature tag.
        // 2. The server then completes the registration and subscription flow.
        // 3. Configure the device via CarrierConfig to enable the preferred access type
        //    feature tag with a numerical value.
        // 4. Trigger IMS registration on the device in an NR network.
        // 5. Verify that the device successfully registers on NR.
        ...
        generator.addMessage(MessageBuildUtils.getDefaultRegister()
                .addRuleSet(new RuleSet.Builder("REGISTER : Numerical access-type tag")
                        .addRule(new RuleSet.Rule.RuleBuilder("Contact")
                                .addContainRule("+g.3gpp.accesstype=\"cellular2\"")
                                .build())
                        .build())
                .build());
        generator.addMessages("<200-REGISTER | >SUBSCRIBE | <200-SUBSCRIBE");
    ...
    @Test
    @P2
    public void register_onNr_withAccessTypeWithoutNumericalValue_verifiesTag()
            throws Exception {
        // 1. Set up the server to expect a REGISTER request and verify its Contact header
        //    includes the '+g.3gpp.accesstype="cellular"' feature tag.
        // 2. The server then completes the registration and subscription flow.
        // 3. Configure the device via CarrierConfig to enable the preferred access type
        //    feature tag without a numerical value.
        // 4. Trigger IMS registration on the device in an NR network.
        // 5. Verify that the device successfully registers on NR.
        ...
        generator.addMessage(MessageBuildUtils.getDefaultRegister()
                .addRuleSet(new RuleSet.Builder("REGISTER : Plain access-type tag")
                        .addRule(new RuleSet.Rule.RuleBuilder("Contact")
                                .addContainRule("+g.3gpp.accesstype=\"cellular\"")
                                .build())
                        .build())
                .build());
        generator.addMessages("<200-REGISTER | >SUBSCRIBE | <200-SUBSCRIBE");
    ...
    public void register_onNr_with305AsPolicy3gpp_triggersDeregistered() throws Exception {
        ...
        // 4. Verify that the device fails registration and enters a deregistered state without
    ...
    public void register_onNr_with403AsRepeatedError_triggersDeregistered()
        ...
        // 2. Configure the device to treat a 403 response as a 'repeated' error type.
        //    For this configuration, the device is expected to deregister without triggering
        //    a PLMN block.
        // 3. Trigger IMS registration on the device.
        // 4. Verify that the device enters a deregistered state without any suggested action.
    ...
    public void register_onNr_with403AsRatBlockError_triggersPlmnBlockWithTimeout()
            throws Exception {
        // 1. Set up the server to respond to the REGISTER request with a 403 Forbidden.
        // 2. Configure the device to treat a 403 response as a 'RAT block' error that
        //    triggers a PLMN block with timeout.
        // 3. Trigger IMS registration on the device.
        // 4. Verify that the device enters a deregistered state with the suggested action to
        ...
        PersistableBundle objExtraRegErrBundle = new PersistableBundle();
        objExtraRegErrBundle.putInt(
                CarrierConfig.Ims.KEY_EXTRA_REG_ERR_FINAL_TYPE_INT, ERROR_TYPE_RAT_BLOCK);
        mConfig.putPersistableBundle(
                CarrierConfig.Ims.KEY_EXTRA_REG_ERR_BUNDLE, objExtraRegErrBundle);
    ...
    public void register_onNr_with500AsRepeatedOnlyAttachedNetwork_triggersPlmnBlockWithTimeout()
        ...
        // 1. Set up the server to respond to the REGISTER request with a 500 Server Internal Error.
        // 2. Configure the device via CarrierConfig to define the 5xx response as a 'repeated
        //    error on attached network' type, which results in blocking the PLMN with a
        //    specific protocol timer.
        // 3. Trigger IMS registration on the device in an NR network.
        // 4. Verify that the device enters a deregistered state with the suggested action to
        //    block the PLMN with a timeout on the NR network.
        ...
        PersistableBundle objExtraRegErrBundle = new PersistableBundle();
        objExtraRegErrBundle.putInt(CarrierConfig.Ims.KEY_EXTRA_REG_ERR_FINAL_TYPE_INT,
                ERROR_TYPE_REPEATED_WITH_ONLY_ATTACHED_NETWORK);
        mConfig.putPersistableBundle(
                CarrierConfig.Ims.KEY_EXTRA_REG_ERR_BUNDLE, objExtraRegErrBundle);
    ...
    public void register_onNr_with503AndTempPlmnBlockRats_triggersPlmnBlockWithTimeout()
            throws Exception {
        // 1. Set up the server to respond to the REGISTER request with a 503 Service Unavailable.
        // 2. Configure the device via carrier config to apply a temporary PLMN block on the
        //    NR network when IMS registration has failed on all available P-CSCFs.
        // 3. Trigger IMS registration on the device in an NR network.
        // 4. Verify that the device enters a deregistered state with the suggested action to
        //    block the PLMN with a timeout.
        ...
        mConfig.putIntArray(
                ...
                new int[] {ACCESS_NETWORK_TYPE_NGRAN});

        PersistableBundle objExtraRegErrBundle = new PersistableBundle();
        objExtraRegErrBundle.putInt(
                CarrierConfig.Ims.KEY_EXTRA_REG_ERR_FINAL_TYPE_INT, ERROR_TYPE_NOT_SPECIFIED);
        mConfig.putPersistableBundle(
                CarrierConfig.Ims.KEY_EXTRA_REG_ERR_BUNDLE, objExtraRegErrBundle);
