...
JniUceService::JniUceService(CBServiceNoti pCBServiceNoti,
                             IN IMS_UINT32 _nSimSlot /* = 0*/)
    : m_nSimSlot(_nSimSlot) {
  IMS_TRACE_D("UCE_M : JniUceService = %" PFLS_u, sizeof(JniUceService), 0, 0);
  //---------------------------------------------------------------------------------------------
  // m_strTarget.Sprintf("%s.UceApp%02d",
  ...
  m_pJniUceServiceThread =
      (JniUceServiceThread *)(IMSProcess::GetInstance()->GetApplicationThread(
          STR_UCE_LISTENER_THREAD_NAME));
  if (m_pJniUceServiceThread != NULL) {
    m_pJniUceServiceThread->SetCallback(reinterpret_cast<IMS_UINTP>(this),
                                        pCBServiceNoti);
  } else {
