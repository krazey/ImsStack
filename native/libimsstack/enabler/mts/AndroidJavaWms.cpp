...
IMS_BOOL androidJavaWms_processSendMOSMS(IN IMS_UINT32 nSmsFormat, IN const ByteArray& objData,
        IN const AString& strAddr, IN IMS_SINT32 nSeqId, IN IMS_SINT32 nSlotId)
{
    ...
    {
        bIsAckorError =
                (((*(objData.GetData()) & 0x07) == 2) || ((*(objData.GetData()) & 0x07) == 4) ||
                        ((*(objData.GetData()) & 0x07) == 6))
                ? IMS_TRUE
                : IMS_FALSE;
    }
...
PRIVATE VIRTUAL AndroidJavaWMS::~AndroidJavaWMS()
{
    IMS_TRACE_D("~AndroidJavaWMS() :: Name(%s)", GetName().GetStr(), 0, 0);
}

PUBLIC GLOBAL AndroidJavaWMS* AndroidJavaWMS::GetAndroidJavaWMS(IN IMS_SINT32 nSlotId)
{
...
PUBLIC GLOBAL void AndroidJavaWMS::DestroyAndroidJavaWMS(IN IMS_SINT32 nSlotId)
{
    if (s_pAndroidJavaWms[nSlotId] != IMS_NULL)
    {
...
PUBLIC GLOBAL IMS_BOOL AndroidJavaWMS::StartUp()
{
    return IMS_TRUE;
}

PUBLIC GLOBAL void AndroidJavaWMS::CleanUp() {}

PUBLIC VIRTUAL IMS_RESULT AndroidJavaWMS::Init()
{
    return IMS_SUCCESS;
}

PUBLIC VIRTUAL IMS_RESULT AndroidJavaWMS::Release()
{
    return IMS_SUCCESS;
}

PUBLIC VIRTUAL IMS_UINT32 AndroidJavaWMS::ReportMtSMS(IN IMS_UINT32 nSmsformat,
        IN IMS_UINT32 nSmslength, IN CONST IMS_BYTE* pbySmsdata, IN IMS_SINT32 nSlotId)
{
    ...
    if (s_pCBJniMtsService != IMS_NULL)
    {
        return (s_pCBJniMtsService(IUMtsService::REPORT_MTS_MT_SMS,
                ...
                ? IMtsClient::MT_FAILURE
                : IMtsClient::MT_SUCCESS;
    }
...
PUBLIC VIRTUAL IMS_RESULT AndroidJavaWMS::ReportMoStatus(IN IMS_UINT32 nReason,
        ...
        IN IMS_SINT32 nSeqId /* = -1 */, IN IMS_SINT32 nSlotId)
{
    ...
    androidJavaWms_sendMsgToJava(
            IUMtsService::REPORT_MTS_MO_STATUS, (IMS_UINTP)pSmsServiceReportMOStatusParam, nSlotId);
    return IMS_SUCCESS;
}

PUBLIC VIRTUAL IMS_BOOL AndroidJavaWMS::OnMessage(IN IMSMSG& objMSG)
{
    ...
    switch (objMSG.nMSG)
    {
        case IUMtsService::NOTI_MTSENABLER_SEND_MO_SMS:
        {
            ...
            androidJavaWms_processSendMOSMS(pParam->nSmsFormat, pParam->objData, pParam->strAddr,
                    pParam->nSeqId, m_nSlotId);
            bResult = IMS_TRUE;
        }
        break;
        default:
