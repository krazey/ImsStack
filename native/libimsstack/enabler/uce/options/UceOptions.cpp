...
PUBLIC
UceOptions::UceOptions(IN CONST AString& strManagerName,
                       IN ICoreService* piCoreService,
                       IN ICapabilities* piCapabilities, IN IMS_UINT32 nKey,
                       IN IMS_BOOL isSendingRequest, IN IMS_SINT32 nSimSlot)
...
PUBLIC
IMS_BOOL UceOptions::SendOptionsRequest(IN AString strRemoteURI,
                                        IN IMS_UINT32 ownCapabilities) {
  IMS_TRACE_D("SendOptionsRequest: remoteUri [%s] ", strRemoteURI.GetStr(), 0,
              0);
...
IMS_BOOL UceOptions::SendOptionsResponse(IN IMS_UINT32 nResponse,
                                         IN AString reason,
                                         IN IMS_UINT32 ownCapabilities) {
  (void)reason;
  if (m_piCapabilities == IMS_NULL) {
...
IMS_UINT32 UceOptions::GetCapability(IMSList<AString> objContactList) {
  ...
  for (IMS_UINT32 i = 0; i < objContactList.GetSize(); i++) {
    ...
    if (featureTag.Contains(
      ...
      IMS_TRACE_I("GetCapability:Uuknow Feature tag[%s]", featureTag.GetStr(),
                  0, 0);
    }
  }
  return capabilities;
...
PROTECTED VIRTUAL void UceOptions::CapabilityQueryDelivered(
    IN ICapabilities* piCapabilities_) {
  ...
  IMessage* piMessage =
      m_piCapabilities->GetPreviousResponse(IMessage::CAPABILITIES_QUERY);
  if (piMessage == IMS_NULL) {
  ...
  IMSList<AString> objContactList =
      piSIPMessage->GetHeaders(ISipHeader::CONTACT_NORMAL);
  IMS_UINT32 capabilities = GetCapability(objContactList);
  SendOptionsResponseInd(piMessage->GetStatusCode(),
                         piMessage->GetReasonPhrase(), capabilities);
  OptionsTerminated();
}

PROTECTED VIRTUAL void UceOptions::CapabilityQueryDeliveryFailed(
    IN ICapabilities* piCapabilities_) {
  // received failure response to options request
  if (piCapabilities_ == IMS_NULL || (m_piCapabilities != piCapabilities_)) {
    IMS_TRACE_I("CapabilityQueryDeliveryFailed:piCapabilities_ is null", 0, 0,
                0);
  ...
  SendOptionsResponseInd(piMessage->GetStatusCode(),
                         piMessage->GetReasonPhrase(), 0);
  OptionsTerminated();
}

PRIVATE
void UceOptions::SetContactHeader(IN IMS_UINT32 capabilities,
...
void UceOptions::SendOptionsResponseInd(IN IMS_SINT32 nResponseCode,
                                        IN AString reason,
                                        IN IMS_UINT32 capabilities) {
  IMS_TRACE_I("SendOptionsResponseInd:key[%d], code[%d], reason[%s]", m_nKey,
              nResponseCode, reason.GetStr());
...
void UceOptions::OptionsTerminated() {
  IMS_TRACE_I("OptionsTerminated:send UCE_OPTIONS_DELETED_IND to manager", 0, 0,
              0);
...
void UceOptions::SetIARIFeatureTag(IN IMS_UINT32 capabilities,
                                   IN AString& strIARITag) {
...
void UceOptions::SetICSIFeatureTag(IN IMS_UINT32 capabilities,
                                   IN AString& strICSITag) {
...
void UceOptions::SetNoTypeFeatureTag(IN IMS_UINT32 capabilities,
                                     IN AString& strTag) {
