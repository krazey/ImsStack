...
PUBLIC
UceOptionsManager::UceOptionsManager(IN CONST AString& _strName,
  ...
  m_nSimSlot = simSlotId;
  IMS_TRACE_D("UCE_M : UceOptionsManager = %" PFLS_u, sizeof(UceOptionsManager),
              0, 0);
  IMS_TRACE_I("UceOptionsManager", 0, 0, 0);
}

PUBLIC VIRTUAL UceOptionsManager::~UceOptionsManager() {
  IMS_TRACE_D("UCE_F : UceOptionsManager = %" PFLS_u, sizeof(UceOptionsManager),
              0, 0);
  IMS_TRACE_I("~UceOptionsManager", 0, 0, 0);
...
PUBLIC
IMS_BOOL UceOptionsManager::SendOptionsRequest(IN IMS_UINT32 nKey,
                                               IN AString strRemoteURI,
  ...
  UceOptions* pOptions = new UceOptions(GetName(), m_piCoreService, IMS_NULL,
                                        nKey, IMS_TRUE, m_nSimSlot);
...
IMS_BOOL UceOptionsManager::SendOptionsResponse(IN IMS_UINT32 nKey,
...
IMS_BOOL UceOptionsManager::ReceivedOptions(IN ICoreService* piCoreService,
  ...
  if (piCoreService == IMS_NULL || piCapabilities ||
      m_piCoreService != piCoreService) {
    IMS_TRACE_I("ReceivedOptions:piCoreService or piCapabilities is null", 0, 0,
                0);
    return IMS_FALSE;
  }
  IMS_UINT32 key = getReceivedKey();

  UceOptions* pOptions = new UceOptions(
      GetName(), m_piCoreService, piCapabilities, key, IMS_FALSE, m_nSimSlot);
  m_objReceivedUceOptionsMap.Add(key, pOptions);

  ISipMessage* piSIPMessage =
      piCapabilities->GetPreviousRequest(IMessage::CAPABILITIES_QUERY)
          ->GetMessage();
...
void UceOptionsManager::AoSConnected() { m_bAoSConnected = IMS_TRUE; }

void UceOptionsManager::AoSDisconnected() { m_bAoSConnected = IMS_FALSE; }
...
void UceOptionsManager::SendOptionsReceivedInd(IN IMS_UINT32 nKey,
                                               IN AString from,
                                               IN IMS_UINT32 capabilities) {
  IMS_TRACE_I("SendOptionsReceivedInd:key[%d], From[%s]", nKey, from.GetStr(),
              0);
...
void UceOptionsManager::SendOptionsCommandError(IN IMS_UINT32 nKey,
                                                IN IMS_UINT32 code) {
