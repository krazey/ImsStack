...
STATE_MSG_ENTRY(PUBLISH_REQUESTED,
                &UcePublishManager::StateIDLE_PublishRequested)
...
STATE_MSG_ENTRY(PUBLISH_REQUESTED,
                &UcePublishManager::StatePUBLISHING_PublishRequested)
STATE_MSG_ENTRY(AOS_DISCONNECTING,
                &UcePublishManager::StatePUBLISHING_AoSDisConnecting)
STATE_MSG_ENTRY(AOS_DISCONNECTED,
                &UcePublishManager::StatePUBLISHING_AoSDisConnected)
STATE_MSG_ENTRY(PUBLISH_SUCCEEDED,
                &UcePublishManager::StatePUBLISHING_Published)
...
STATE_MSG_ENTRY(PUBLISH_REQUESTED,
                &UcePublishManager::StatePUBLISHED_PublishRequested)
STATE_MSG_ENTRY(AOS_DISCONNECTING,
                &UcePublishManager::StatePUBLISHED_AoSDisconnecting)
STATE_MSG_ENTRY(AOS_DISCONNECTED,
                &UcePublishManager::StatePUBLISHED_AoSDisconnected)
STATE_MSG_ENTRY(PUBLISH_TERMINATED, &UcePublishManager::StateALL_Terminated)
STATE_MSG_ENTRY(PUBLISH_REFRESH_STARTED,
                &UcePublishManager::StatePUBLISHED_RefreshStarted)
STATE_MSG_ENTRY(SERVICE_CLOSED, &UcePublishManager::StateALL_Terminated)
END_STATE_MSG_MAP()

BEGIN_STATE_MSG_MAP(UcePublishManager, REFRESHING)
STATE_MSG_ENTRY(PUBLISH_REQUESTED,
                &UcePublishManager::StateREFRESHING_PublishRequested)
STATE_MSG_ENTRY(AOS_DISCONNECTING,
                &UcePublishManager::StateREFRESHING_AoSDisConnecting)
STATE_MSG_ENTRY(AOS_DISCONNECTED,
                &UcePublishManager::StateREFRESHING_AoSDisConnected)
STATE_MSG_ENTRY(PUBLISH_SUCCEEDED,
                &UcePublishManager::StateREFRESHING_Refreshed)
STATE_MSG_ENTRY(PUBLISH_FAILED,
                &UcePublishManager::StateREFRESHING_RefreshFailed)
STATE_MSG_ENTRY(PUBLISH_TERMINATED, &UcePublishManager::StateALL_Terminated)
STATE_MSG_ENTRY(PUBLISH_REFRESHED,
                &UcePublishManager::StateREFRESHING_Refreshed)
STATE_MSG_ENTRY(PUBLISH_REFRESH_FAILED,
                &UcePublishManager::StateREFRESHING_RefreshFailed)
STATE_MSG_ENTRY(SERVICE_CLOSED, &UcePublishManager::StateALL_Terminated)
END_STATE_MSG_MAP()
...
STATE_MSG_ENTRY(PUBLISH_REQUESTED,
                &UcePublishManager::StateTERMINATING_PublishRequested)
STATE_MSG_ENTRY(AOS_DISCONNECTED,
                &UcePublishManager::StateTERMINATING_AoSDisconnected)
STATE_MSG_ENTRY(PUBLISH_SUCCEEDED,
                &UcePublishManager::StateTERMINATING_Published)
...
-------------------------------------------------------------------------------------------------
*/
UcePublishManager::UcePublishManager(IN ICoreService *_piCoreService,
  ...
  IMS_TRACE_I("UCE_M : UcePublishManager = %" PFLS_u, sizeof(UcePublishManager),
              0, 0);
...
UcePublishManager::~UcePublishManager() {
  IMS_TRACE_I("UCE_F : UcePublishManager = %" PFLS_u, sizeof(UcePublishManager),
              0, 0);
  StopTimer(TIMER_ALL);
  DestroyPublication();
}
...
IMS_RESULT UcePublishManager::MessageMediator_AdjustMessage(
  ...
  if (piSIPMsg->GetMethod().Equals(SipMethod::PUBLISH) == IMS_FALSE) {
    IMS_TRACE_I("MessageMediator_AdjustMessage:it is not publish method.", 0, 0,
                0);
    return IMS_SUCCESS;
  }
  AString strContactHeader = piSIPMsg->GetHeader(ISipHeader::CONTACT_NORMAL);
  if (UceConfig::GetInstance()->GetBoolValue(
          UceConfig::KEY_USE_CONTACT_HEADER_IN_PUBLISH, m_nSimSlot) ==
      IMS_FALSE) {
...
IMS_BOOL UcePublishManager::SendPublishRequest(IN IMS_UINT32 key,
                                               IN AString pidfXml,
                                               IN AString eTag,
                                               IN IMS_UINT32 capability,
                                               IN IMS_UINT32 extended) {
  IMS_TRACE_D("SendPublishRequest:key [%d], extended[%d], capability[%d]", key,
              extended, capability);
  ...
  IMSMSG objMsg(PUBLISH_REQUESTED, 0,
...
void UcePublishManager::PublicationDelivered(IN IPublication *piPublication) {
  ...
  if (piPublication != m_piPublication) {
    IMS_TRACE_I("PublicationDelivered:IPublication from Engine is not mine.", 0,
                0, 0);
    return;
  }
...
void UcePublishManager::PublicationDeliveryFailed(
    IN IPublication *piPublication) {
  IMS_TRACE_I("PublicationDeliveryFailed", 0, 0, 0);
  if (piPublication != m_piPublication) {
    IMS_TRACE_I(
        "PublicationDeliveryFailed:IPublication from Engine is not mine.", 0, 0,
        0);
    return;
  }
...
void UcePublishManager::PublicationTerminated(IN IPublication *piPublication) {
  ...
  if (piPublication != m_piPublication) {
    IMS_TRACE_I("PublicationTerminated:IPublication from Engine is not mine.",
                0, 0, 0);
    return;
  }
...
void UcePublishManager::PublicationRefreshStarted(
    IN IPublication *piPublication) {
  IMS_TRACE_I("PublicationRefreshStarted", 0, 0, 0);
  if (piPublication != m_piPublication) {
    IMS_TRACE_I(
        "PublicationRefreshStarted:IPublication from Engine is not mine.", 0, 0,
        0);
    return;
  }
...
void UcePublishManager::PublicationRefreshCompleted(
    IN IPublication *piPublication) {
  IMS_TRACE_I("PublicationRefreshCompleted", 0, 0, 0);
  if (piPublication != m_piPublication) {
    IMS_TRACE_I(
        "PublicationRefreshCompleted:IPublication from Engine is not mine.", 0,
        0, 0);
    return;
  }
  ...
  if (piSIPMessage != IMS_NULL) {
    ...
    IMS_TRACE_D("PublicationRefreshCompleted:StatusCode[%d], reason[%s]",
                nResponseCode, piSIPMessage->GetReasonPhrase().GetStr(), 0);
  }

  if (SipStatusCode::IsFinalSuccess(nResponseCode)) {
    IMS_TRACE_I("PublicationRefreshCompleted:success refresh PUBLISH.", 0, 0,
                0);
...
IMS_BOOL UcePublishManager::StateIDLE_PublishRequested(IN IMSMSG &objMsg) {
  ...
  if (pPublicationData == IMS_NULL) {
    IMS_TRACE_I("StateIDLE_PublishRequested - pPublicationData is null", 0, 0,
                0);
    return IMS_FALSE;
  } else {
...
IMS_BOOL UcePublishManager::StateIDLE_AoSConnected(IN IMSMSG &objMsg) {
  (void)objMsg;
  IMS_TRACE_I("StateIDLE_AoSConnected:nConnectedApps [%08x]",
              m_nConnectedServices, 0, 0);
...
IMS_BOOL UcePublishManager::StateON_PublishRequested(IN IMSMSG &objMsg) {
  ...
  IPublicationData *pPublicationData = (IPublicationData *)objMsg.nLparam;
  if (pPublicationData == IMS_NULL) {
    IMS_TRACE_E(0, "StateON_PublishRequested:IPublicationData is null", 0, 0,
                0);
    return IMS_FALSE;
  }
  if (CreatePublication() == IMS_FALSE) {
    IMS_TRACE_E(0, "StateON_PublishRequested:create publication failed", 0, 0,
                0);
    SendPublishCommandErrorInd(pPublicationData->m_nKey,
                               IUUceService::COMMAND_CODE_GENERIC_FAILURE);
    delete pPublicationData;
  ...
  if (SetPublish(INITIAL) == IMS_FALSE) {
    IMS_TRACE_E(0, "StateON_PublishRequested:SetPublish(INITIAL) is failed", 0,
                0, 0);
    SendPublishCommandErrorInd(m_nKey,
                               IUUceService::COMMAND_CODE_GENERIC_FAILURE);
    return IMS_FALSE;
  }
...
IMS_BOOL UcePublishManager::StatePUBLISHING_PublishRequested(
    IN IMSMSG &objMsg) {
  IMS_TRACE_I("StatePUBLISHING_PublishCmd ", 0, 0, 0);
  IPublicationData *pPublicationData = (IPublicationData *)objMsg.nLparam;
  if (pPublicationData == IMS_NULL) {
    IMS_TRACE_I("StatePUBLISHING_PublishCmd - pPublicationData is null", 0, 0,
                0);
    return IMS_FALSE;
  }
  SendPublishCommandErrorInd(pPublicationData->m_nKey,
...
IMS_BOOL UcePublishManager::StatePUBLISHING_AoSDisConnecting(
    IN IMSMSG &objMsg) {
  (void)objMsg;
  ...
  if (piMessage == IMS_NULL) {
    ...
    if (pResponseData != IMS_NULL) {
      SendPublishResponseInd(m_nKey, pResponseData->m_nResponseCode,
                             pResponseData->m_strReason,
                             pResponseData->m_nReasonCause,
...
IMS_BOOL UcePublishManager::StatePUBLISHING_Failed(IN IMSMSG &objMsg) {
  ...
  SendPublishResponseInd(
      m_nKey, pResponseData->m_nResponseCode, pResponseData->m_strReason,
...
IMS_BOOL UcePublishManager::StatePUBLISHED_PublishRequested(IN IMSMSG &objMsg) {
  ...
  IPublicationData *pPublicationData = (IPublicationData *)objMsg.nLparam;
  if (pPublicationData == IMS_NULL) {
    IMS_TRACE_E(0, "StatePUBLISHED_PublishRequested:IPublicationData is null",
                0, 0, 0);
    return IMS_FALSE;
  }
  m_nKey = pPublicationData->m_nKey;
  ...
  delete pPublicationData;
  if (CreatePublication() == IMS_FALSE) {
    IMS_TRACE_E(0, "StatePUBLISHED_PublishRequested:create publication failed",
                0, 0, 0);
    SendPublishCommandErrorInd(m_nKey,
                               IUUceService::COMMAND_CODE_GENERIC_FAILURE);
    return IMS_FALSE;
  ...
  if (SetPublish(INITIAL) == IMS_FALSE) {
    IMS_TRACE_E(0,
                "StatePUBLISHED_PublishRequested:SetPublish(INITIAL) is failed",
                0, 0, 0);
    SendPublishCommandErrorInd(m_nKey,
                               IUUceService::COMMAND_CODE_GENERIC_FAILURE);
    return IMS_FALSE;
  ...
  if (Publish() == IMS_TRUE) {
    ...
    IMS_TRACE_E(0, "StatePUBLISHED_PublishRequested:Publish() is failed", 0, 0,
                0);
    SendPublishCommandErrorInd(m_nKey,
                               IUUceService::COMMAND_CODE_GENERIC_FAILURE);
  }
...
IMS_BOOL UcePublishManager::StatePUBLISHED_AoSDisconnecting(IN IMSMSG &objMsg) {
  ...
  if (CreatePublication() == IMS_TRUE) {
    ...
    IMS_TRACE_E(0, "StatePUBLISHED_AoSDisconnecting:Sending UnPublish Failed.",
                0, 0, 0);
  }
  SetState(ON);
  DestroyPublication();
...
IMS_BOOL UcePublishManager::StateREFRESHING_PublishRequested(
    IN IMSMSG &objMsg) {
  (void)objMsg;
  IMS_TRACE_I("StateREFRESHING_PublishRequested ", 0, 0, 0);
  IPublicationData *pPublicationData = (IPublicationData *)objMsg.nLparam;
  if (pPublicationData == IMS_NULL) {
    IMS_TRACE_I("StateREFRESHING_PublishRequested - pPublicationData is null",
                0, 0, 0);
    return IMS_FALSE;
  }
  if (m_bReceivedUnPublishRequest == IMS_TRUE) {
...
IMS_BOOL UcePublishManager::StateREFRESHING_AoSDisConnecting(
    IN IMSMSG &objMsg) {
  (void)objMsg;
...
IMS_BOOL UcePublishManager::StateTERMINATING_PublishRequested(
    IN IMSMSG &objMsg) {
  IMS_TRACE_I("StateTERMINATING_PublishRequested", 0, 0, 0);
  IPublicationData *pPublicationData = (IPublicationData *)objMsg.nLparam;
  if (pPublicationData == IMS_NULL) {
    IMS_TRACE_I("StateTERMINATING_PublishRequested - pPublicationData is null",
                0, 0, 0);
    return IMS_FALSE;
  } else {
    SendPublishCommandErrorInd(pPublicationData->m_nKey,
...
IMS_BOOL UcePublishManager::StateTERMINATING_AoSDisconnected(
    IN IMSMSG &objMsg) {
  (void)objMsg;
...
IMS_BOOL UcePublishManager::StateALL_Terminated(IN IMSMSG &objMsg) {
  (void)objMsg;
  IMS_TRACE_I("StateALL_Terminated:current State[%s]", StateToString(m_eState),
              0, 0);
  StopTimer(TIMER_ALL);
  DestroyPublication();
  switch (GetState()) {
    case PUBLISHING:
      ...
      break;
    case REFRESHING:
      SendPublishResponseInd(m_nKey, SipStatusCode::SC_504, "Server Time-out",
                             0, "", "");
      break;
    case TERMINATING:
      SendPublishResponseInd(m_nKey, SipStatusCode::SC_504, "Server Time-out",
...
void UcePublishManager::LoadConfigValue() {
  ...
  m_objExponentialRetryTimeSec =
      UceConfig::GetInstance()->GetExponentialRetryPublishRespTimeArray(
          m_nSimSlot);
}

IPublishResponseData *UcePublishManager::GetPublishResponseData(
...
ISipMessage *UcePublishManager::GetISIPMessage(
    IMS_BOOL bRequireResponseMessage) {
  if (m_piPublication != IMS_NULL) {
    IMessage *piMessage = IMS_NULL;
    if (bRequireResponseMessage == GET_MESSAGE_FROM_RESPONSE) {
      piMessage =
          m_piPublication->GetPreviousResponse(IMessage::PUBLICATION_PUBLISH);
...
void UcePublishManager::SendPublishCommandErrorInd(IMS_UINT32 nKey,
                                                   IMS_UINT32 nCommandError) {
  IMS_TRACE_I("SendPublishCommandErrorInd:key[%d], error[%d]", nKey,
              nCommandError, 0);
  ...
  if (pParam != IMS_NULL) {
    ...
    IMS_TRACE_E(
        0, "SendPublishCommandErrorInd:Command Error Param creation is failed",
        0, 0, 0);
  }
  m_nKey = 0;
}

void UcePublishManager::SendPublishResponseInd(
    IMS_UINT32 nKey, IMS_SINT32 nResponseCode, AString strReason,
    IMS_SINT32 nReasonHeaderCause, AString strReasonHeaderText, AString eTag,
    IMS_BOOL bNeedToRetry) {
  IMS_TRACE_I("SendPublishResponseInd:key[%d], responseCode[%d]", nKey,
              nResponseCode, 0);
  IMS_TRACE_I("SendPublishResponseInd:reason[%s], Cause[%d], Text[%s]",
              strReason.GetStr(), nReasonHeaderCause,
              strReasonHeaderText.GetStr());
  m_bUnpublishSent = IMS_FALSE;
  if (nKey == 0) {
    ...
    if (pParam != IMS_NULL) {
      ...
      IMS_TRACE_E(0,
                  "SendPublishResponseInd:UpdatedInd Param creation is failed",
                  0, 0, 0);
    }
  } else {
    ...
    if (pParam != IMS_NULL) {
      ...
      IMS_TRACE_E(0,
                  "SendPublishResponseInd:ResponseInd Param creation is failed",
                  0, 0, 0);
    }
  }
  m_nKey = 0;
...
IMS_BOOL UcePublishManager::SetPublish(IN IMS_BOOL bIsRefresh,
                                       AString strMinExpiryValue) {
  IMS_TRACE_I("SetPublish:value[%d]-(0)initial, (1)refresh, MinExpiryValue[%s]",
              bIsRefresh, strMinExpiryValue.GetStr(), 0);
  // set header
  ...
  if (m_strEtag != IMS_NULL && m_strEtag.IsEmpty() == IMS_FALSE) {
    ...
    IMS_TRACE_I("InitPublish:add SIP IF MATCH header with [%s]",
                m_strEtag.GetStr(), 0, 0);
  }
  SetRefreshPolicy(piMessage, strMinExpiryValue);
  // check ATT/TMUS live network and equipment test
  piMessage->AddHeader(ISipHeader::ACCEPT_CONTACT,
                       "*;+g.oma.sip-im;explicit;require");
...
void UcePublishManager::SetRefreshPolicy(ISipMessage *piMessage,
                                         AString strMinExpiryValue) {
  if (piMessage == IMS_NULL) {
  ...
  if (strMinExpiryValue.IsEmpty() != IMS_TRUE) {
    piMessage->AddHeader(ISipHeader::EXPIRES_ANY, strMinExpiryValue,
                         AString::ConstNull());
    IMS_TRACE_I("Min Expire value [ %s ]", strMinExpiryValue.GetStr(), 0, 0);
  } else {
    if (!strExpireValue.EqualsIgnoreCase("0")) {
      piMessage->AddHeader(ISipHeader::EXPIRES_ANY, strExpireValue,
                           AString::ConstNull());
  ...
  if (nRefreshRatio == 0) {
    ...
    m_piPublication->SetRefreshPolicy(IPublication::REFRESH_POLICY_RATIO,
                                      strExpireValue.ToInt32(), nRefreshRatio,
                                      nRefreshRatio);
  }
}
...
IMS_BOOL UcePublishManager::Publish() {
  ...
  if (m_piPublication->Publish(ByteArray::ConstNull(), AString::ConstNull()) !=
      IMS_SUCCESS) {
    DestroyPublication();
...
IMS_BOOL UcePublishManager::SetPidfXmlBody(ISipMessage *piMessage) {
  ...
  if (m_strPidfXml != IMS_NULL && !m_strPidfXml.IsEmpty()) {
    const IMS_BYTE *pszXML =
        reinterpret_cast<const IMS_BYTE *>(m_strPidfXml.GetStr());
    objContent.Attach(pszXML, m_strPidfXml.GetLength());
  } else {
    IMS_TRACE_E(0, "[ERROR]SetPidfXmlBody:m_strPidfXml is null or empty", 0, 0,
                0);
    return IMS_FALSE;
  }

  // TMO TRD 2020 4Q - GID-MTRREQ-480586
  if (UceConfig::GetInstance()->GetBoolValue(UceConfig::KEY_ENCODE_PUBLISH_BODY,
                                             m_nSimSlot) == IMS_TRUE &&
      m_bReceivedFailResponse == IMS_FALSE) {
...
void UcePublishManager::GetEtagAndExpireValue(ISipMessage *piMessage) {
  ...
  if (szETagHeader.IsNULL() != IMS_TRUE) {
    ISipHeader *piHeader =
        SipParsingHelper::CreateHeader(ISipHeader::SIP_ETAG, szETagHeader);
...
IMS_BOOL UcePublishManager::HandleFailResponse(IMS_SINT32 nResponseCode) {
  ...
  if (UceConfig::GetInstance()->IsImsRegistrationRequired(
          IMS_TRUE, nResponseCode, m_nSimSlot)) {
    SetState(ON);
    Process403Scenario();
    DestroyPublication();
    return IMS_FALSE;
  ...
  switch (UceConfig::GetInstance()->GetPublishRetryType(nResponseCode,
                                                        m_nSimSlot)) {
...
void UcePublishManager::SetPublishStateToAoS(IN IMS_UINT32 nState) {
  if (nState == PUBLISH_STARTED && m_bSetPublishStarted == IMS_FALSE) {
    ...
    IMS_TRACE_D("SetPublishStateToAoS:send Publish Started Msg to UceApp", 0, 0,
                0);
    ...
    MessageService::PostMessage(m_strAppName, objMsg);
    m_bSetPublishStarted = IMS_FALSE;
    IMS_TRACE_D("SetPublishStateToAoS:send Publish Terminated Msg to UceApp", 0,
                0, 0);
  }
}
...
IMS_BOOL UcePublishManager::ProcessRetryAfterHeader() {
  ...
  if (strHeader.GetLength() <= 0) {
    IMS_TRACE_I("ProcessRetryAfterHeader:RETRY_AFTER_SEC value less than 0.", 0,
                0, 0);
    return IMS_FALSE;
  }
  ISipHeader *piHeader =
  ...
  piHeader->Destroy();
  IMS_TRACE_I("ProcessRetryAfterHeader:Retry After Header value[%d]", nValue, 0,
              0);
...
#define NOT_AUTHORIZED_FOR_PRESENCE "NOT AUTHORIZED FOR PRESENCE"
  ...
  if (strReasonPhrs.Contains(NOT_AUTHORIZED_FOR_PRESENCE) == IMS_TRUE) {
    IMS_TRACE_I("ReasonPhrase:Not Authorized for presence (ReasonPhrase)", 0, 0,
                0);
    return IMS_TRUE;
  }
  ...
  for (IMS_UINT32 i = 0; i < objReasonList.GetSize(); i++) {
    ...
    if (strReason.Contains(NOT_AUTHORIZED_FOR_PRESENCE) == IMS_TRUE) {
      IMS_TRACE_I("ReasonPhrase:Not Authorized for presence(ReasonPhrase)", 0,
                  0, 0);
      return IMS_TRUE;
    }
  }
...
IMS_BOOL UcePublishManager::Process423Scenario(IMS_BOOL bIsRefresh) {
  ...
  ISipHeader *piHeader =
      SipParsingHelper::CreateHeader(ISipHeader::MIN_EXPIRES, szMinExpire);
  if (piHeader == IMS_NULL) {
  ...
  piHeader->Destroy();
  IMS_TRACE_I("Process423Scenario:min Expire Value [%s]", szMinExpire.GetStr(),
              0, 0);
  DestroyPublication();
...
IMS_BOOL UcePublishManager::ProcessImmediatelyRetryResponseScenario() {
  if (m_nImmediatelyRetryCount >=
      UceConfig::GetInstance()->GetIntValue(
          UceConfig::KEY_IMMEDIATELY_RETRY_PUBLISH_RESPONSE_MAX_COUNT,
          m_nSimSlot)) {
    IMS_TRACE_I(
        "ProcessImmediatelyRetryResponseScenario:over max "
...
IMS_BOOL UcePublishManager::ProcessExponentialRetryResponseScenario() {
  ...
  if (nExponentialRetryMaxCount != 0) {
    if (m_nExponentialRetryCount >= nExponentialRetryMaxCount) {
      IMS_TRACE_I("ProcessExponentialRetryResponseScenario:over max MaxCount",
                  0, 0, 0);
      return IMS_FALSE;
    }
  } else {
...
IMS_BOOL UcePublishManager::StartTimer(INTERNAL_TIMER eTimer,
                                       IMS_UINT32 nTime) {
  switch (eTimer) {
...
void UcePublishManager::StopTimer(INTERNAL_TIMER eTimer) {
  ...
  IMS_TRACE_I(
      "StopTimer:type[%d]-0(exponential), 1(retry), 2(retry_after),3(all)",
      eTimer, 0, 0);
  switch (eTimer) {
    case TIMER_EXPONENTIAL: {
      if (m_pExponentialTimer == IMS_NULL) {
...
void UcePublishManager::HandleExponentialRetryTimer() {
  IMS_TRACE_I("HandleExponentialRetryTimer:Current State[%d]",
              StateToString(m_eState), 0, 0);
  if (GetState() == PUBLISHING) {
    if (RetryPublish(INITIAL) == IMS_TRUE) {
      return;
    }
...
void UcePublishManager::HandleRetryTimer() {
  IMS_TRACE_I("HandleRetryTimer:Current State[%d]", StateToString(m_eState), 0,
              0);
  if (GetState() == PUBLISHING) {
    if (RetryPublish(INITIAL) == IMS_TRUE) {
      return;
...
void UcePublishManager::HandleRetryAfterTimer() {
  IMS_TRACE_I("HandleRetryAfterTimer:Current State[%d]",
              StateToString(m_eState), 0, 0);
  if (GetState() == PUBLISHING) {
    if (RetryPublish(INITIAL) == IMS_TRUE) {
      return;
...
IMS_BOOL UcePublishManager::RetryPublish(IMS_BOOL bRefresh,
                                         AString strMinExpiryValue) {
  if (CreatePublication() == IMS_FALSE) {
    return IMS_FALSE;
  }
...
const IMS_CHAR *UcePublishManager::StateToString(IMS_UINT32 _eState) {
  static const char *szState[] = {
      "IDLE",       "ON",          "PUBLISHING", "PUBLISHED",
      "REFRESHING", "TERMINATING", "ERROR",
  };
