...
*/
PUBLIC
UceService::UceService(IN CONST AString& strAppName,
                       IN CONST IMS_SINT32 nSlotId)
...
PROTECTED VIRTUAL void UceService::CoreService_PageMessageReceived(
  ...
  IMS_TRACE_I("CoreService_PageMessageReceived:invalid case- send 480 response",
              0, 0, 0);
  //---------------------------------------------------------------------------------------------
  (void)piService;
...
void UceService::CoreService_ReferenceReceived(IN ICoreService* piService,
  ...
  IMS_TRACE_I("CoreService_ReferenceReceived:invalid case : send 480 response",
              0, 0, 0);
  //---------------------------------------------------------------------------------------------
  (void)piService;
...
void UceService::CoreService_ServiceClosed(IN ICoreService* piService,
  ...
  if (m_piCoreService != piService) {
    IMS_TRACE_D("CoreService_ServiceClosed:ICoreService is not matched", 0, 0,
                0);
    return;
  }
...
void UceService::CoreService_SessionInvitationReceived(
  ...
  IMS_TRACE_I(
      "CoreService_SessionInvitationReceived:invalid case : send 480 response",
      0, 0, 0);
  //---------------------------------------------------------------------------------------------
  (void)piService;
...
void UceService::CoreService_CapabilityQueryReceived(
  ...
  if (m_piCoreService != piService) {
    IMS_TRACE_D(
        "CoreService_CapabilityQueryReceived:ICoreService is not matched", 0, 0,
        0);
    return;
  }
...
void UceService::EnableManager() {
  ...
  if (m_pUcePublishManager == IMS_NULL) {
    m_pUcePublishManager =
        new UcePublishManager(m_piCoreService, m_strAppName, m_nSlotId);
  }

  if (m_pUceOptionsManager == IMS_NULL) {
    m_pUceOptionsManager =
        new UceOptionsManager(m_strAppName, m_piCoreService, m_nSlotId);
  }
}
...
void UceService::EnableCoreService() {
  ...
  m_piCoreService = DYNAMIC_CAST(
      ICoreService*,
      (Connector::Open("imscore", ImsServiceConfig::GetAppName(ImsAppId::UCE),
                       strServiceID)));
...
IMS_BOOL UceService::SendOptionsResponse(IUceOptionsRespCmdPrm* pParam) {
  ...
  if (m_pUceOptionsManager != IMS_NULL) {
    m_pUceOptionsManager->SendOptionsResponse(
        pParam->m_nKey, pParam->m_nResponseCode, pParam->m_strReason,
        pParam->m_nMyCaps);
  }
  delete pParam;
  return IMS_TRUE;
...
IMS_BOOL UceService::SendPublishRequest(IN IUcePubCmdPrm* pParam) {
  ...
  if (m_pUcePublishManager != IMS_NULL) {
    m_pUcePublishManager->SendPublishRequest(
        pParam->m_nKey, pParam->m_strPidfXml, pParam->m_strEtag,
        pParam->m_nCapability, pParam->m_nExtended);
  }
  delete pParam;
  return IMS_TRUE;
}
...
IMS_BOOL UceService::QuerySingleCapability(IN IUceSingleSubCmdPrm* pParam) {
  ...
  if (m_pUceSubscribeManager != IMS_NULL) {
    m_pUceSubscribeManager->QuerySingleCapability(pParam->m_strUser,
                                                  pParam->m_nKey);
  }
  delete pParam;
  return IMS_TRUE;
...
IMS_BOOL UceService::QueryMultiCapability(IN IUceListSubCmdPrm* pParam) {
  ...
  if (m_pUceSubscribeManager != IMS_NULL) {
    m_pUceSubscribeManager->QueryMultiCapability(pParam->userList,
                                                 pParam->m_nKey);
  }
  delete pParam;
  return IMS_TRUE;
