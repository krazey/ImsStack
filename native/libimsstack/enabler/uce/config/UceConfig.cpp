...
PUBLIC
UceConfig::UceConfig() : m_objAssetMap(IMSMap<IMS_SINT32, UceAssetItems*>()) {
...
void UceConfig::Init(IN IMS_SINT32 nSimSlot) {
  ICarrierConfig* piCc =
      ConfigService::GetConfigService()->GetCarrierConfig(nSimSlot);
  piCc->AddListener(this);

  Update(piCc, nSimSlot);
}

AString UceConfig::GetAStringValue(IN KEY_UCE_STRING eKey,
  ...
  IMS_TRACE_D("GetAStringValue:simSlot[%d], RLS_URI, value[%d]", nSimSlot,
              strRet.GetStr(), 0);
  return strRet;
}
...
IMS_UINT32 UceConfig::GetIntValue(IN KEY_UCE_INT eKey, IN IMS_SINT32 nSimSlot) {
  ...
  IMS_TRACE_D("GetIntValue:simSlot[%d], key[%s], value[%d]", nSimSlot,
              GetKeyString(eKey), nRet);
  return nRet;
}
...
IMS_BOOL UceConfig::GetBoolValue(IN KEY_UCE_BOOL eKey, IN IMS_SINT32 nSimSlot) {
  ...
  IMS_TRACE_D("GetBoolValue:simSlot[%d], key[%s], value[%d]", nSimSlot,
              GetKeyString(eKey), bRet);
  return bRet;
}

IMSVector<IMS_SINT32> UceConfig::GetExponentialRetryPublishRespTimeArray(
    IN IMS_SINT32 nSimSlot) {
  if (m_objAssetMap.GetIndexOfKey(nSimSlot) < 0) {
    return IMSVector<IMS_SINT32>();
  }
  IMSVector<IMS_SINT32> objExponentialRetryPublishResponseTimeSec =
      m_objAssetMap.GetValue(nSimSlot)
          ->m_objExponentialRetryPublishResponseTimeSec;
  return objExponentialRetryPublishResponseTimeSec;
}

IMS_UINT32 UceConfig::GetPublishRetryType(IN IMS_SINT32 nResponseCode,
                                          IN IMS_SINT32 nSimSlot) {
  ...
  IMS_TRACE_D("GetPublishRetryType:simSlot[%d], respoonseCode[%d]", nSimSlot,
              nResponseCode, 0);
...
IMS_BOOL UceConfig::IsImsRegistrationRequired(IN IMS_BOOL isPublish,
...
PUBLIC VIRTUAL void UceConfig::CarrierConfig_NotifyConfigChanged(
    IN IMS_SINT32 nSlotId) {
  ICarrierConfig* piCc =
      ConfigService::GetConfigService()->GetCarrierConfig(nSlotId);
  Update(piCc, nSlotId);
}
...
void UceConfig::Update(IN ICarrierConfig* piCc, IN IMS_SINT32 nSimSlot) {
  ...
  objUceAssetItems->m_nExtendedExpireValuePublish = piCc->GetInt(
      CarrierConfig::ImsUce::KEY_EXTENDED_EXPIRE_VALUE_PUBLISH_SEC_INT);
  objUceAssetItems->m_nPublishRefreshRatio =
      piCc->GetInt(CarrierConfig::ImsUce::KEY_PUBLISH_REFRESH_RATIO_INT);
  objUceAssetItems->m_nExpireValueListSubscribe = piCc->GetInt(
      CarrierConfig::ImsUce::KEY_EXPIRE_VALUE_LIST_SUBSCRIBE_SEC_INT);
  objUceAssetItems->m_strRlsUri =
      piCc->GetString(CarrierConfig::ImsUce::KEY_RLS_URI_STRING);
  objUceAssetItems->m_bSubscribeIndepedentOfPublish = piCc->GetBoolean(
      CarrierConfig::ImsUce::KEY_SUBSCRIBE_INDEPENDENT_OF_PUBLISH_BOOL);
  ...
  objUceAssetItems->m_bUseContactHeaderInPublish = piCc->GetBoolean(
      CarrierConfig::ImsUce::KEY_USE_CONTACT_HEADER_IN_PUBLISH_BOOL);
  objUceAssetItems->m_bUseContactHeaderInSubscribe = piCc->GetBoolean(
      CarrierConfig::ImsUce::KEY_USE_CONTACT_HEADER_IN_SUBSCRIBE_BOOL);
  ...
  objUceAssetItems->m_nImmediatelyRetryPublishResponseMaxCount =
      piCc->GetInt(CarrierConfig::ImsUce::
                       KEY_IMMEDIATELY_RETRY_PUBLISH_RESPONSE_MAX_COUNT_INT);
  ...
  objUceAssetItems->m_nRetryPublishResponseMaxCount =
      piCc->GetInt(CarrierConfig::ImsUce::
                       KEY_FIXED_TIME_RETRY_PUBLISH_RESPONSE_MAX_COUNT_INT);
  objUceAssetItems->m_nRetryPublishResponseTimeSec =
      piCc->GetInt(CarrierConfig::ImsUce::
                       KEY_FIXED_TIME_RETRY_PUBLISH_RESPONSE_TIME_SEC_INT);
  objUceAssetItems->m_objExponentialRetryPublishResponse =
      piCc->GetIntArray(CarrierConfig::ImsUce::
                            KEY_VARIABLE_TIME_RETRY_PUBLISH_RESPONSE_INT_ARRAY);
  objUceAssetItems->m_nExponentialRetryPublishResponseMaxCount =
      piCc->GetInt(CarrierConfig::ImsUce::
                       KEY_VARIABLE_TIME_RETRY_PUBLISH_RESPONSE_MAX_COUNT_INT);
  objUceAssetItems->m_objExponentialRetryPublishResponseTimeSec =
      piCc->GetIntArray(
          CarrierConfig::ImsUce::
              KEY_VARIABLE_TIME_RETRY_PUBLISH_RESPONSE_TIME_SEC_INT_ARRAY);
  objUceAssetItems->m_objReAttemptRegistrationPublishResponse =
      piCc->GetIntArray(
          CarrierConfig::ImsUce::
              KEY_REATTEMPT_REGISTRATION_PUBLISH_RESPONSE_INT_ARRAY);
  objUceAssetItems->m_objReAttemptRegistrationSubscribeResponse =
      piCc->GetIntArray(
          CarrierConfig::ImsUce::
              KEY_REATTEMPT_REGISTRATION_SUBSCRIBE_RESPONSE_INT_ARRAY);
