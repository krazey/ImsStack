...
UceXmlDocumentHelperThread::UceXmlDocumentHelperThread(
  ...
  m_pXMLTransactionProvider =
      XmlFactory::GetInstance()->CreateTransactionProvider();
...
PUBLIC
IMS_BOOL UceXmlDocumentHelperThread::Start(IN CONST AString& strName,
                                           IN IMS_UINT32 nIndex) {
  m_nIndex = nIndex;
  IMS_CHAR szIndex[10];
  ...
  if (m_piThread != IMS_NULL) {
    IMS_TRACE_I("Start : thread (%s) is already running.",
                m_strThreadName.GetStr(), 0, 0);
    return IMS_TRUE;
  }

  m_piThread =
      ThreadService::GetThreadService()->Create(m_strThreadName, m_nSimSlot);

  if (m_piThread == IMS_NULL) {
    IMS_TRACE_I("Start : creating a thread (%s) failed.",
                m_strThreadName.GetStr(), 0, 0);
    return IMS_FALSE;
  }
  Initialize();
  ...
  if (m_piThread->Activate() == IMS_FALSE) {
    IMS_TRACE_I("Start : starting a thread [%s] failed.",
                m_strThreadName.GetStr(), 0, 0);
    return IMS_FALSE;
  }
...
void UceXmlDocumentHelperThread::SendMsg(IN IMS_UINT32 nMSG,
                                         IN IMS_UINTP nWparam,
                                         IN IMS_UINTP nLparam) {
  m_piThread->PostMessageI(nMSG, nWparam, nLparam);
}

PUBLIC VIRTUAL IMS_RESULT
UceXmlDocumentHelperThread::XmlTransaction_NotifyParsingCompleted(
  ...
  if (piXMLTxn == IMS_NULL || piXMLTxn != piXMLTransaction) {
    IMS_TRACE_E(0, "[ERROR]XML_ParseCompleted - No matched XML transaction", 0,
                0, 0);
    return eResult;
  }
  ...
  if (piResponse->GetResponseCode() == IXmlResponse::RESPONSE_CODE_SUCCESS) {
    ...
    IMS_TRACE_D("XML_ParseCompleted:strRootName [%s]", strRootName.GetStr(), 0,
                0);
...
PUBLIC VIRTUAL IMS_BOOL UceXmlDocumentHelperThread::Initialize() {
  ...
  m_objMessageMap.Add(IMS_MSG_START,
                      reinterpret_cast<msgHandler>(
                          &UceXmlDocumentHelperThread::StartMessageHandler));
  m_objMessageMap.Add(
      IMS_MSG_TERMINATE,
      reinterpret_cast<msgHandler>(
          &UceXmlDocumentHelperThread::TerminateMessageHandler));
  m_objMessageMap.Add(
      MSG_THREAD_RECEIVED_NOTIFY_RLMI,
      reinterpret_cast<msgHandler>(
          &UceXmlDocumentHelperThread::ReceivedRlmiNotifyMessageHandler));
  m_objMessageMap.Add(
      MSG_THREAD_PARSERED_XML_RLMI,
      reinterpret_cast<msgHandler>(
          &UceXmlDocumentHelperThread::ParsedRlmiXmlMessageHandler));
  return IMS_FALSE;
}
...
PUBLIC VIRTUAL void UceXmlDocumentHelperThread::Uninitialize() {
  ...
  if (m_pXMLTransactionProvider != IMS_NULL) {
    XmlFactory::GetInstance()->DestroyTransactionProvider(
        m_pXMLTransactionProvider);
...
PROTECTED VIRTUAL IMS_BOOL
UceXmlDocumentHelperThread::OnStart(IN IMSMSG& objMSG) {
  //---------------------------------------------------------------------------------------------
...
PROTECTED VIRTUAL IMS_BOOL
UceXmlDocumentHelperThread::OnTerminate(IN IMSMSG& objMSG) {
  //---------------------------------------------------------------------------------------------
...
PROTECTED VIRTUAL IMS_BOOL
UceXmlDocumentHelperThread::OnMessage(IN IMSMSG& objMSG) {
  //---------------------------------------------------------------------------------------------
...
PRIVATE VIRTUAL IMS_BOOL
UceXmlDocumentHelperThread::Runnable_Run(IN IMSMSG& objMSG) {
  ...
  if (nIndexOfFunc < 0) {
    IMS_TRACE_I("Runnable_Run:This message is not supported.", objMSG.GetName(),
                0, 0);
    return IMS_TRUE;
  }
...
IMS_RESULT UceXmlDocumentHelperThread::XMLDataTokenization(
    IN CONST ByteArray& objBytes) {
  IMS_TRACE_D("XMLDataTokenization()", 0, 0, 0);
  IXmlRequest* piXMLRequest = IMS_NULL;
  //---------------------------------------------------------------------------------------------
  IXmlTransaction* piXMLTransaction =
      m_pXMLTransactionProvider->CreateTransaction();
...
IMS_BOOL UceXmlDocumentHelperThread::ReceivedRlmiNotifyMessageHandler(
    IMSMSG& objMsg) {
  m_pUceNotifyMessageBody = (UceNotifyMessageBody*)objMsg.nLparam;
  if (m_pUceNotifyMessageBody == IMS_NULL) {
    IMS_TRACE_I("ReceivedRlmiNotifyMessageHandler:UceNotifyMessageBody is null",
                0, 0, 0);
    IMS_MSG_CreateNPostActivityMessageByName(
        m_strQueryName, IUUceService::UCE_XML_PARSE_COMPLETED_IND,
        &m_objNonCapabilities, &m_objPidfXmls);
    return IMS_TRUE;
  }
  /*
  ...
  */
  ...
  AString strStart;
  IMS_TRACE_D("ReceivedRlmiNotifyMessageHandler:Content-Type[%s]",
              strContentType.GetStr(), 0, 0);

  ISipHeader* piHeader =
      SipParsingHelper::CreateHeader(ISipHeader::CONTENT_TYPE, strContentType);
  if (piHeader != IMS_NULL) {
    const SipParameter* pParameter = piHeader->GetParameter("start");
    if (pParameter != IMS_NULL) {
      strStart = pParameter->GetValue();
      if (strStart.StartsWith('\"') == IMS_TRUE &&
          strStart.EndsWith('\"') == IMS_TRUE) {
        strStart = strStart.GetSubStr(1, strStart.GetLength() - 2);
      } else if (strStart.StartsWith('<') == IMS_TRUE &&
                 strStart.EndsWith('>') == IMS_TRUE) {
        strStart = strStart.GetSubStr(1, strStart.GetLength() - 2);
      }
      IMS_TRACE_D("ReceivedRlmiNotifyMessageHandler:start=[%s]",
                  strStart.GetStr(), 0, 0);
  ...
  if (bMatchingTheResult == IMS_FALSE) {
    IMS_TRACE_I(
        "ReceivedRlmiNotifyMessageHandler:Don`t match Any content type or id",
        0, 0, 0);
    IMS_MSG_CreateNPostActivityMessageByName(
        m_strQueryName, IUUceService::UCE_XML_PARSE_COMPLETED_IND,
        &m_objNonCapabilities, &m_objPidfXmls);
...
IMS_BOOL UceXmlDocumentHelperThread::ParsedRlmiXmlMessageHandler(
    IMSMSG& objMsg) {
  (void)objMsg;
  ...
  for (IMS_UINT32 i = 0; i < m_objRlmiCidList.GetSize(); ++i) {
    for (IMS_UINT32 j = 0; j < m_objBodyParts.GetSize(); ++j) {
      ...
      if (strConID.StartsWith('\"') == IMS_TRUE &&
          strConID.EndsWith('\"') == IMS_TRUE) {
        strConID = strConID.GetSubStr(1, strConID.GetLength() - 2);
      } else if (strConID.StartsWith('<') == IMS_TRUE &&
                 strConID.EndsWith('>') == IMS_TRUE) {
        strConID = strConID.GetSubStr(1, strConID.GetLength() - 2);
      }
      IMS_TRACE_D("ParsedRlmiXmlMessageHandler:content Id [%s]",
                  strConID.GetStr(), 0, 0);
      if (strConID.Contains(m_objRlmiCidList.GetAt(i)) == IMS_TRUE) {
        IMS_TRACE_D("ParsedRlmiXmlMessageHandler:find content id.index[%d]", j,
                    0, 0);
        ...
        IMS_TRACE_D(
            "ParsedRlmiXmlMessageHandler:current remain bodyPart`s size[%d]",
            m_objBodyParts.GetSize(), 0, 0);
        return IMS_TRUE;
      }
    }
...
void UceXmlDocumentHelperThread::SendParseCompletedMsg(
    IMS_SINT32 eXMLInfoType) {
      ...
      if (piChildNode->GetNodeType() == INode::ELEMENT_NODE) {
        ...
        if (piChildElement->GetTagName().Equals("instance")) {
          ...
          if (temp.GetLength() == 0) {
            ...
            } else if (temp.StartsWith('<') == IMS_TRUE &&
                       temp.EndsWith('>') == IMS_TRUE) {
              temp = temp.GetSubStr(1, temp.GetLength() - 2);
            }
